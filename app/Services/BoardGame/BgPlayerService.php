...
class BgPlayerService
{
    ...
    public static function joinTheGame($user, $slug)
    {
        if (!$user) {
            return [
                'status' => 'error',
                'status_message' => __('notReceived.not_received_user')
            ];
        }
        ...
        if (!$slug) {
            return [
                'status' => 'error',
                'status_message' => __('notReceived.not_received_slug')
            ];
        }
        ...
        $boardGame = BoardGame::query()
            ->findBySlug($slug)
            ->with([
                'players' => function ($q) use ($user) {
                    $q->findByUserId($user->id);
                },
                'settings'
            ])
            ->first();
        ...
        if (!$boardGame) {
            return [
                'status' => 'error',
                'status_message' => __('boardGame.not_found'),
            ];
        }

        if ($player) {
            return ErrorService::message(__('boardGame.player.already_in_event'));
        }
        ...
        if ($typeSettings === 'registrationIsClose') {
            return [
                'status' => 'error',
                'status_message' => __('boardGame.registration_is_close'),
            ];
        }

        return DB::transaction(function () use ($user, $boardGame, $typeSettings) {
            ...
            $fields = [
                ...
                'not_active_reason' => $typeSettings ? ($typeSettings->value === 'upon-request' ? __('boardGame.player.on_moderate') : null) : null,
                'created_by' => $user->id,
            ];

            $newPlayer = BoardGamePlayer::create($fields);

            switch ($typeSettings->value ?? null) {
                case 'upon-request':
                    $returnMessage = __('boardGame.player.request_on_moderate');
                    $logMessage = __('boardGame.player.applied_to_participate_in_the_event');
                    break;
                default:
                    $returnMessage = __('boardGame.player.success_registration_in_event');
                    $logMessage = __('boardGame.player.joined_in_event');
                    break;
            }
